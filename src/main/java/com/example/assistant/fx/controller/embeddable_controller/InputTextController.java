...
import com.example.assistant.model.InputText;
...
public class InputTextController {
    ...
    public void addInputOperation(ActionEvent event) {
        ...
        if (isValidNameField(nameField) && isValidNameElement(elementField)) {
            ...
            inputText.setName(nameField.getText());
            inputText.setSelector(elementField.getText());
            inputText.setInputText(inputTextField.getText());
            inputText.setPush(pushEnter.isSelected());

            trackingObservable.addTrackingForOperation(inputText, choiceBoxTracking);

            operationObservable.addOperation(inputText);
        }
    }
